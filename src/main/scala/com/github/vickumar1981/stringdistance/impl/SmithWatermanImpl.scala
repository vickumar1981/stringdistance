...
private[stringdistance]trait SmithWatermanImpl extends GapSubstitution {
  ...
  def smithWatermanGotoh[T](s1: Array[T], s2: Array[T], gap: ConstantGap = ConstantGap()): Double = {
  ...
  // scalastyle:off
  private def calculateSmithWaterman[T](s1: Array[T], s2: Array[T], gap: Gap, windowSize: Int): Double = {
  ...
  private def calculateSmithWatermanGotoh[T](s1: Array[T], s2: Array[T], gap: ConstantGap): Double = {
    val (s1Len, s2Len) = (s1.length, s2.length)
