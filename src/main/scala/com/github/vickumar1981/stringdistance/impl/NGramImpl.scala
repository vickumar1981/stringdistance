...
trait NGramImpl[T] extends NGramTokenizer[T] {
  protected def nGram(s1: Array[T], s2: Array[T], n: Int = 1): Double = {
    require(n > 0, "Ngram score, n-gram size must be a positive number.")
    foldNGram(s1, s2, n)(0d)(_ => 1d) {
      (s1Tok, s2Tok, dist) => dist.toDouble / math.max(s1Tok.length, s2Tok.length)
    }
  }

  protected def nGramDist(s1: Array[T], s2: Array[T], n: Int = 1): Int = {
