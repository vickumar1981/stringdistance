    ...
    if (c == '\0' && r.length == 0) o
      ...
      val t = {
        (c: @annotation.switch) match {
          ...
          case 'h' =>
            if ((l.length >= 1 && lowercaseVowel.contains(l.last) && (r.length == 0 || !lowercaseVowel.contains(r.head)))
              || (l.length >= 2 && l.last == 'h'
          ...
          case 'v' => shift(1, o :+ 'f')
          case 'w' | 'y' => if (r.length == 0 || !lowercaseVowel.contains(r.head)) shift(1, o) else shift(1, o :+ c)
